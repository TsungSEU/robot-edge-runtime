...
### 3.2 端侧推理功能
#### 3.2.1 模块结构
- **core/**：核心规划器（auto_planner, humanoid_planner, planner_factory）
- **agents/**：RL 推理代理（auto_ppo_agent, humanoid_ppo_agent, onnx_inference_engine）
- **observation/**：状态表示（humanoid_state_traits, auto_state_traits, state_converter）
- **config/**：配置管理（planner_config_manager）
- **maps/**：代价地图与价值评估
- **safety/**：语义约束与安全检查
- **utils/**：工具函数
#### 3.2.2 Agent 组件
- **auto_ppo_agent**：Auto 模式 PPO 代理（25-dim state, 4 discrete actions）
- **humanoid_ppo_agent**：Humanoid 模式 PPO 代理（43-dim state, 3 continuous actions）
- **onnx_inference_engine**：ONNX Runtime 推理引擎

## 4. 技术规格

### 4.1 环境规格
- **Auto 模式状态维度**：25维向量（24 + 1 reachability）
- **Humanoid 模式状态维度**：43维向量
- **Auto 动作维度**：4维离散动作空间
- **Humanoid 动作维度**：3维连续动作空间
